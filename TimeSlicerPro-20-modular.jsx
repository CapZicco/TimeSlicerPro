...
function selectRadial(index, total) {
    ...
    var cx = w / 2;
    var cy = h / 2;
    var r = Math.max(w, h) * 1.1; // ensure covers all corners
...
// Robust Diagonal Selection
function selectInclined(i, total) {
    if (typeof i !== "number" || typeof total !== "number" || total < 1 || isNaN(i) || isNaN(total)) return;
    var doc = app.activeDocument;
    var w = parseFloat(doc.width);
    var h = parseFloat(doc.height);
    var xStep = w / total;
...
// Generic Polygon Selection (helper)
