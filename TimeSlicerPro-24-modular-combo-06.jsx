/*
 * TimeSlicerPro v24.06 "starter"
 * CapZicco & Copilot + progressive Window
 * Modular, robust, streamlined GUI, centralized main loop, step-by-step counter, teamwork ready.
 */

// === VERSION CHECK (Photoshop compatibility) ===
...
function ProgressWindow(totalSteps) {
    ...
    this.label = this.win.add("statictext", undefined, "Starting...");
    ...
    this.update = function(step, message) {
        step = parseInt(step, 10);
        ...
        if (message) {
            ...
            this.label.text = step + " of " + totalSteps;
        }
        this.win.update();
    };
    this.close = function () {
...
// === PATTERN ENGINE: all step-by-step pattern functions ===
// === PATTERN ENGINE: tutte le funzioni con contatore ===
...
function runMosaicGrid(files, settings, progressWin) {
    ...
    for (var cell = 0; cell < totalCells; cell++) {
        if (progressWin) progressWin.update(cell + 1, "Mosaic Grid: cella " + (cell + 1) + "/" + totalCells);
...
function runRadialSlices(files, settings, progressWin) {
    ...
    for (var i = 0; i < numSlices; i++) {
        if (progressWin) progressWin.update(i + 1, "Radial Slices: fetta " + (i + 1) + "/" + numSlices);
...
function runSpiralStripes(files, settings, progressWin) {
    ...
    for (var i = 0; i < numStripes; i++) {
        if (progressWin) progressWin.update(i + 1, "Spiral Stripes: stripe " + (i + 1) + "/" + numStripes);
...
function runVerticalSlices(files, settings, direction, progressWin) {
    ...
    for (var i = 0; i < numSlices; i++) {
        if (progressWin) progressWin.update(i + 1, "Vertical Slices: slice " + (i + 1) + "/" + numSlices);
...
function runHorizontalSlices(files, settings, direction, progressWin) {
    ...
    for (var i = 0; i < numSlices; i++) {
        if (progressWin) progressWin.update(i + 1, "Horizontal Slices: slice " + (i + 1) + "/" + numSlices);
...
function runDiagonalSlices(files, settings, direction, progressWin) {
    ...
    for (var i = 0; i < numSlices; i++) {
        if (progressWin) progressWin.update(i + 1, "Diagonal Slices: slice " + (i + 1) + "/" + numSlices);
        var sliceIdx = (direction === "RightToLeft") ? (numSlices - 1 - i) : i;
        var processingFile = openFile(files[sliceIdx].path + "/" + files[sliceIdx].name);
        if (!processingFile) continue;
        duplicateLayersInto(newFile);
...
function runOvalCircularSlices(files, settings, direction, progressWin) {
    ...
    for (var i = 0; i < numSlices; i++) {
        if (progressWin) progressWin.update(i + 1, "Oval-Circular: slice " + (i + 1) + "/" + numSlices);
...
function runEquilateralTriangles(files, settings, progressWin) {
    // Open base file and get dimensions
    var baseFile = openFile(files[0].path + "/" + files[0].name);
    if (!baseFile) return null;
    var newFile = baseFile.duplicate();
    var w = newFile.width.value || parseInt(newFile.width);
    ...
    var sideNet = side - gap;
    if (sideNet < 5) sideNet = 5;
    var triHeight = sideNet * Math.sqrt(3) / 2;

    var stepX = sideNet + gap;
    var stepY = triHeight + gap;

    var rows = Math.ceil(h / stepY) + 2;
    var cols = Math.ceil(w / stepX) + 2;
    var imgCount = files.length;

    // Only count actually visible triangles
    var realTriangles = [];
    for (var row = 0; row < rows; row++) {
        for (var col = 0; col < cols; col++) {
            var x0 = col * stepX + ((row % 2) ? stepX / 2 : 0);
            var y0 = row * stepY;
            var points;
            if ((row + col) % 2 === 0) {
                points = [
                    [x0, y0 + triHeight],
                    [x0 + sideNet / 2, y0],
                    [x0 + sideNet, y0 + triHeight]
                ];
            } else {
                points = [
                    [x0, y0],
                    [x0 + sideNet, y0],
                    [x0 + sideNet / 2, y0 + triHeight]
                ];
            }
            if (isTriangleVisible(points, w, h)) {
                realTriangles.push({row: row, col: col, points: points});
            }
        }
    }

    var total = realTriangles.length;
    if (isNaN(total) || total < 1) total = 1;
    if (!progressWin) progressWin = new ProgressWindow(total);

    updateProgress(progressWin, 0, total);

    for (var t = 0; t < total; t++) {
        var tri = realTriangles[t];
        var imgIdx = t % imgCount;

        updateProgress(progressWin, t + 1, total);
        ...
        try {
            ...
            $.writeln('DEBUG: error during refresh: ' + e);
        }
    }
    updateProgress(progressWin, total, total);
        ...
        try {
            ...
            if (e.number && (e.number & 0xFFFF) === 8007) {
                ...
                if (choice === "exit") {
                    ...
                    continue; // Back to main menu (restart main loop)
...
// === MODULAR AND STREAMLINED GUI/UI ===
