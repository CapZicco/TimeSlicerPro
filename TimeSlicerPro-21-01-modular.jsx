/*
 * TimeSlicerPro v21.00
 * by CapZicco & Copilot
 * - Funzionalità Oval, Spiral, Grid Slices (Mosaic), e nuovo pattern Wave Slices!
 * - UI aggiornata: sezione Wave Slices con slider per onde e ampiezza
 * - Gestione robusta di ESC/usrcancel (errore 8007)
 * - Patch: tooltip grid, main loop, tutte le funzioni V20 mantenute
 */
...
(function() {
    ...
    function getMyFiles(sourceFolder, extList) {
        ...
        var len = docs.length;
        // --- PRIMA del ciclo for! ---
if (settings.outputPattern === "Grid Slices (Mosaic)") {
    var newFile = runMosaicGrid(files, settings);
    ...
    repeat = confirm("Do you want to generate another slicing?", true);
    continue; // Salta il ciclo for, passa subito alla prossima iterazione di while
}

// --- SOLO SE NON È MOSAIC, esegui il ciclo for normale! ---
for (var i = 0; i < len; i++) {
    // ... (resto del ciclo per gli altri pattern)
}
            var doc = docs[i];
    ...
    function selectRadial(index, total) {
        if (typeof index !== "number" || typeof total !== "number" || total < 1 || isNaN(index) || isNaN(total)) return;
        var doc = app.activeDocument;
        var w = parseFloat(doc.width);
        var h = parseFloat(doc.height);
        var cx = w / 2;
        var cy = h / 2;
        var r = Math.max(w, h) * 1.1;
    ...
    function selectInclined(i, total) {
        var doc = app.activeDocument;
        var w = parseInt(doc.width);
        var h = parseInt(doc.height);
        var d = parseInt(w + h);
    ...
    // === NUOVO PATTERN: WAVE SLICES ===
    function selectWaveSlice(index, total, waveCount, amplitude) {
        var doc = app.activeDocument;
        var w = parseFloat(doc.width);
        var h = parseFloat(doc.height);
        ...
        var yStep = h / total;
        var y0 = index * yStep;
        var y1 = (index + 1) * yStep;

        var points = [];
        var steps = 64;
        // Top wave (da sinistra a destra)
        for (var i = 0; i <= steps; i++) {
            var t = i / steps;
            var x = t * w;
            var theta = t * waveCount * 2 * Math.PI + (index % 2) * Math.PI; // alterna la fase
            var y = y0 + Math.sin(theta) * amplitude / 2;
            points.push([x, y]);
        }
        // Bottom wave (da destra a sinistra)
        for (var i = steps; i >= 0; i--) {
            var t = i / steps;
            var x = t * w;
            var theta = t * waveCount * 2 * Math.PI + (index % 2) * Math.PI;
            var y = y1 + Math.sin(theta) * amplitude / 2;
            points.push([x, y]);
        }
        points.push([0, y0 + Math.sin((index % 2) * Math.PI) * amplitude / 2]);
        selectPolygon(points);
    }
    ...
    function writeLog(msg, outputFolder, enabled) {
        ...
        txtFile.open("a");
        txtFile.writeln("v21 | " + datetime + ": " + msg);
        txtFile.close();
    }
    ...
    function showDialog(settings) {
        var prefs = settings;
        var myWin = new Window("dialog", "TimeSlicerPro v21.00");
        ...
        // WAVE PANEL
        var wavePanel = myWin.SettingsPanel.add("panel", undefined, "Wave Slices Options");
        wavePanel.orientation = "row";
        wavePanel.alignChildren = "left";
        var waveCountGroup = wavePanel.add("group");
        waveCountGroup.add("statictext", undefined, "Number of Waves:");
        var waveCountSlider = waveCountGroup.add("slider", undefined, parseInt(prefs.waveCount) || 3, 1, 12);
        waveCountSlider.preferredSize.width = 100;
        var waveCountValue = waveCountGroup.add("statictext", undefined, (parseInt(prefs.waveCount) || 3).toString());
        waveCountSlider.onChanging = function() {
            waveCountValue.text = Math.round(waveCountSlider.value).toString();
        };
        var waveAmplitudeGroup = wavePanel.add("group");
        waveAmplitudeGroup.add("statictext", undefined, "Amplitude:");
        var waveAmplitudeSlider = waveAmplitudeGroup.add("slider", undefined, parseInt(prefs.waveAmplitude) || 40, 10, 100);
        waveAmplitudeSlider.preferredSize.width = 100;
        var waveAmplitudeValue = waveAmplitudeGroup.add("statictext", undefined, (parseInt(prefs.waveAmplitude) || 40).toString());
        waveAmplitudeSlider.onChanging = function() {
            waveAmplitudeValue.text = Math.round(waveAmplitudeSlider.value).toString();
        };
        wavePanel.visible = OutputPattern.selection && OutputPattern.selection.text == "Wave Slices";
        ...
        OutputPattern.onChange = function () {
            ...
            wavePanel.visible = this.selection && this.selection.text == "Wave Slices";
        ...
        gridRowsSlider.onChanging = function () {
            ...
            gridRowsSlider.helpTip = "The number of images used will be the number found in the folder. If you select more grid cells than available images, images will be reused cyclically.";
            gridColsSlider.helpTip = "The number of images used will be the number found in the folder. If you select more grid cells than available images, images will be reused cyclically.";
        };
        ...
        Button_Group.saveSettingsBtn.onClick = function () {
            ...
            settings.waveCount = Math.round(waveCountSlider.value);
            settings.waveAmplitude = Math.round(waveAmplitudeSlider.value);
        ...
        Button_Group.loadSettingsBtn.onClick = function () {
            ...
            try {
                ...
                gridColsValue.text = Math.round(gridColsSlider.value).toString();
                waveCountSlider.value = Number(settings.waveCount) || 3;
                waveCountValue.text = Math.round(waveCountSlider.value).toString();
                waveAmplitudeSlider.value = Number(settings.waveAmplitude) || 40;
                waveAmplitudeValue.text = Math.round(waveAmplitudeSlider.value).toString();
                updateImageCount(Input1Edit.text);
                myWin.layout.layout(true);
                myWin.update();
    ...
    continue;
                    ...
                    switch (settings.outputPattern) {
                        ...
                        case "Spiral Stripes":
                            ...
                            break;
                        case "Wave Slices":
                            var waveCount = parseInt(settings.waveCount) || 3;
                            var amplitude = parseInt(settings.waveAmplitude) || 40;
                            selectWaveSlice(i, len, waveCount, amplitude);
                            break;
                        default:
