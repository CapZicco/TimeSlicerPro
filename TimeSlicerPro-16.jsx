/*
 * TimeSlicerPro v16.x - FULL SCRIPT
 * by CapZicco & Copilot
 * Migliorie:
 * - closeFile, saveImage unificate
 * - path synthesis ottimizzata
 * - Spiral Stripes: slider twist/numero strisce, visibili nella dialog solo se scelto
 * - Salvataggio con timestamp (output non sovrascrive)
 * - Angle nei settings sempre "NN°"
 * - Aggiorna numero file trovati anche dopo load settings
 * - Ripeti mantiene i settings precedenti
 */

// =========================
// 1. FILESYSTEM & DOCUMENT UTILITIES OTTIMIZZATE
// =========================
...
// saveImage unificata (JPG/PNG) con timestamp
...
var tsVersion = "16.0-SPIRAL-PRO";

var TSclass = function () {};

TSclass.prototype.main = function () {
    var repeat = true;
    if (!TSclass.p) { // Solo il primo giro: setta i default
    ...
    while (repeat) {
        ...
        // NON chiamare più setPreferences qui!
...
/* =========================
 * 3. SLICING ENGINE PATCH: include Spiral Stripes
 * =========================
 */
...
// =========================
// 4. MASKING, SELECTIONS, SPIRAL STRIPES, SHADOW
// =========================
...
TSclass.setPreferences = function () {
    if (typeof TSclass.p !== "object" || TSclass.p === null) TSclass.p = {};
    var def = {
        inputFolder: "~",
        outputFolder: "~",
        outputFile: "output",
    ...
    for (var k in def) {
        if (!TSclass.p.hasOwnProperty(k) || TSclass.p[k] === undefined || TSclass.p[k] === null) {
            TSclass.p[k] = def[k];
        }
    }
...
TSclass.dialogAndSetOptions = function () {
    var prefs = TSclass.p;
    var myWin = new Window("dialog", "TimeSlicerPro v" + (typeof tsVersion !== "undefined" ? tsVersion : "16"));
    ...
    // --- SPIRAL PANEL (solo se Spiral Stripes) ---
    ...
    var spiralNumSlider = spiralNumGroup.add("slider", undefined, prefs.spiralStripesNum || 12, 2, 64);
    spiralNumSlider.preferredSize.width = 120;
    var spiralNumValue = spiralNumGroup.add("statictext", undefined, (prefs.spiralStripesNum || 12).toString());
    ...
    var spiralTwistSlider = spiralTwistGroup.add("slider", undefined, prefs.stripeTwist || 4, 0, 12);
    spiralTwistSlider.preferredSize.width = 120;
    var spiralTwistValue = spiralTwistGroup.add("statictext", undefined, (prefs.stripeTwist || 4).toString());
    ...
    var angleValue = angleGroup.add("statictext", undefined, (prefs.shadowAngle || 120) + "°");
