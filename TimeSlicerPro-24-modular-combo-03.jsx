/*
 * TimeSlicerPro v24.03 "starter"
 * CapZicco & Copilot + progressive Window
 * Modular, robust, GUI snella, main loop centralizzato, contatore step-by-step, pronto teamwork.
 */
// === VERSION CHECK (compatibilità PS) ===
...
// === PROGRESS WINDOW (AVANZAMENTO) ===

function ProgressWindow(totalSteps) {
    this.win = new Window("palette", "Avanzamento TimeSlicerPro", undefined, {closeButton: false});
    this.win.orientation = "column";
    this.label = this.win.add("statictext", undefined, "Inizio...");
    this.progress = this.win.add("progressbar", undefined, 0, totalSteps);
    this.progress.preferredSize = [300, 20];
    this.win.show();

    // POSIZIONA IN BASSO A DESTRA, COMPATIBILE
    ...
    this.update = function (step, text) {
...
function isTriangleVisible(points, w, h) {
    // Ritorna true se almeno un vertice è dentro l'area immagine (0,0)-(w,h)
...
function runEquilateralTriangles(files, settings, progressWin) {
    ...
    for (var row = 0; row < rows; row++) {
        for (var col = 0; col < cols; col++) {
            if (progressWin) progressWin.update(triIdx + 1, "Triangolo " + (triIdx + 1) + " di " + total);

            var imgIdx = triIdx % imgCount;
            // Calcola la posizione di partenza del triangolo
            ...
            // Salta triangoli completamente fuori dall'immagine
...
function main() {
    ...
    steps = files.length;
            ...
            switch (pattern) {
				...
				default:
                    alert("Pattern non implementato: " + pattern);
                    writeLog("Pattern non implementato: " + pattern, settings.outputFolder, settings.generateLog);
            }
