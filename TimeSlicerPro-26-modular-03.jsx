...
// Salvataggio automatico dei settings
...
btnSaveSettings.onClick = function() {
    var settings = collectSettingsFromGui();
    var defaultName = lastSettingsFileName.replace(".txt", ""); // "TSP-LastSettingsUsed"
    var folder = Folder(settingsFolderPath);
    if (!folder.exists) folder.create();
    var oldFolder = Folder.current;
    Folder.current = folder;
    var file = File.saveDialog("Save settings as...", defaultName + ".txt", "*.txt");
    Folder.current = oldFolder;
    if (!file) return;
    file.open("w");
    for (var k in settings)
...
function applySettingsToGui(settings) {
    ...
    if (txtSourceDir && settings.sourceDir) txtSourceDir.text = settings.sourceDir;
    if (txtDestDir && settings.destDir) txtDestDir.text = settings.destDir;
...
// === [ RUN (VERE) – Classic Horizontal e Vertical ] ===
