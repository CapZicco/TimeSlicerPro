...
function checkUserInterrupt() {
    try {
        // Questo controlla se è stato premuto ESC
        if (ScriptUI.environment.keyboardState.keyName === "Escape") {
            TSP_USER_INTERRUPT = true;
        }
    } catch (e) {
        // Alcune versioni di ExtendScript potrebbero non supportare keyboardState; ignora l'errore
    }

    if (TSP_USER_INTERRUPT) {
        var choice = confirmSpecialExit();
        if (choice === "exit") {
            throw new Error("USER_INTERRUPT_EXIT");
        } else if (choice === "menu") {
            throw new Error("USER_INTERRUPT_MENU");
        } else if (choice === "continue") {
            TSP_USER_INTERRUPT = false;
            // Semplicemente riprendi il loop
        }
    }
}
...
function runClassicHorizontal() {
    try {
        ...
        for (var i = 0; i < numSlices; i++) {
            checkUserInterrupt(); // <--- AGGIUNGI QUI
