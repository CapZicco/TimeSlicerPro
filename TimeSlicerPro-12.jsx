...
// MODIFICA v12: main in loop finché l'utente vuole "ancora slicing"
TSclass.prototype.main = function () {
    var repeat = true;
    while (repeat) {
        // All'inizio di ogni ciclo, resettiamo il flag
        TSclass.p = TSclass.p || {};
...
TSclass.SliceIt = function () {
    ...
    // MODIFICA v12: chiedi se vuoi continuare
    if (confirm("Vuoi generare un altro slicing?", true)) {
...
TSclass.dialogAndSetOptions = function () {
    ...
    // --- LOG CHECKBOX (aggiunta v11) ---
    var logGroup = myWin.add("group", undefined, "");
    logGroup.orientation = "row";
    var logCheckbox = logGroup.add("checkbox", undefined, "Genera file di LOG");
    logCheckbox.value = prefs.generateLog || false;
    logCheckbox.helpTip = "Se selezionato, verrà creato un file di log delle operazioni.";
    ...
    Button_Group.okBtn.onClick = function () {
        ...
        // --- LOG checkbox: salviamo la preferenza SOLO in sessione
...
// PATCH LOG: scrive log quando serve!
TSclass.myAlert = function (msg, title, isErr) {
    TSclass.writeTextFile(msg);
    // alert(msg, title || "Log", !!isErr);
};
// PATCH: scrive il log solo se la checkbox è spuntata
